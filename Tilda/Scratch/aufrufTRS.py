...
dll = ctypes.CDLL('D:\Workspace\Eclipse\Tilda\Tilda\\binary\\NiFpgaUniversalInterfaceDll.dll')

'''
first using TRSwrapper.c commands
'''
...
'''
NiFpga_TRS_IndicatorU16_seqState = {'ref': 0x8146, 'val': ctypes.c_uint16()}
NiFpga_TRS_ControlU16_cmdByHost= {'ref': 0x8142, 'val': ctypes.c_uint16()}

status = dll.NiFpga_Initialize()
print(status)
BitfilePath = ctypes.create_string_buffer(b'D:\\Workspace\\Eclipse\\Tilda\\TildaTarget\\bin\\TimeResolvedSequencer\\NiFpga_TRS.lvbitx')
BitfileSignature = ctypes.create_string_buffer(b'BF31570369009FA00617B7055FD697C8')
resource = ctypes.create_string_buffer(b'Rio1')
session = ctypes.c_ulong()
print(session)
status = dll.NiFpga_Open(BitfilePath, BitfileSignature, resource, 1, ctypes.byref(session))
print(status)
print(session)
