...
import Tilda.PolliFit.Analyzer as Analyzer
from Tilda.PolliFit.TildaTools import select_from_db
...
def absolute_frequency(db, fit1, run1, fit2, run2):
    ...
    energCenter1_d = ((velCenter1_d ** 2 * (isoMass1 * Physics.u) ** 2 * velCenter1 ** 2 / (
            Physics.qe ** 2 * (1 - velCenter1 ** 2 / Physics.c ** 2) ** 3)) + (
                              Physics.c ** 4 * (isoMass1_d * Physics.u) ** 2 / Physics.qe ** 2 * (-1 + (1 / (1 - velCenter1 ** 2 / Physics.c ** 2) ** 0.5)) ** 2)) ** 0.5
    ...
    energCenter2_d = ((velCenter2_d ** 2 * (isoMass2 * Physics.u) ** 2 * velCenter2 ** 2 / (
            Physics.qe ** 2 * (1 - velCenter2 ** 2 / Physics.c ** 2) ** 3)) + (
                              Physics.c ** 4 * (isoMass2_d * Physics.u) ** 2 / Physics.qe ** 2 * (-1 + (1 / (1 - velCenter2 ** 2 / Physics.c ** 2) ** 0.5)) ** 2)) ** 0.5
