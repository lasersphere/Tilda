...
import threading
import math
import Tilda.Driver.TritonListener.Backend.triton_trans
...
class TritonServerUDP:
    ...
    def _remote_call(self,tritoncall, addr, msg_port, msgid):
        ...
        repbytes = Tilda.Driver.TritonListener.Backend.triton_trans.TritonTransmission.serialize_udp_reply(msgid, reply)
                ...
                if msg_type == SERVER_CONF.MSG_ACK:
                    ...
                    if msgid not in self.recmsgids:  # checks for duplicate messages
                        ...
                        if msg_type == SERVER_CONF.MSG_DATA:  #
                            tritonmsg = Tilda.Driver.TritonListener.Backend.triton_trans.TritonTransmission.unserialize(data[19:])
                            self.threadpoolexecutor.submit(self.receive, tritonmsg[0], tritonmsg[1], tritonmsg[2],
                                                           tritonmsg[3])
                        elif msg_type == SERVER_CONF.MSG_BIG:
                            bigid = Tilda.Driver.TritonListener.Backend.triton_trans.BigUDPTransmission.get_big_id_from_bytes(data)

                            if bigid in bigtransms:
                                bigtransms[bigid].add_data(data)
                            else:
                                bigtransms[bigid] = Tilda.Driver.TritonListener.Backend.triton_trans.BigUDPTransmission(data)

                            if bigtransms[bigid].is_complete():
                                complbigtrans = bigtransms[bigid]
                                assembled_trans = complbigtrans.assemble_transmission()
                                tritonmsg = Tilda.Driver.TritonListener.Backend.triton_trans.TritonTransmission.unserialize(assembled_trans)
                            ...
                            tritoncall = Tilda.Driver.TritonListener.Backend.triton_trans.TritonTransmission.unserialize(data[19:])
                            self._remote_call(tritoncall,addr,msg_port, msgid)
                        elif msg_type == SERVER_CONF.MSG_REPLY:
                            reply = Tilda.Driver.TritonListener.Backend.triton_trans.TritonTransmission.unserialize(data[19:])
    ...
    def send(self, name, t, ch, val):
        ...
        self.listlock.release()
        data = Tilda.Driver.TritonListener.Backend.triton_trans.TritonTransmission.serialize_sub(name, t, ch, val)
        for sub in sublist:
    ...
    def subscribeTo(self, ip, port):
        ...
        return Tilda.Driver.TritonListener.Backend.tritonremoteobject.TritonRemoteObject(self, ip, port)
