...
class TritonServerHybrid(Tilda.Driver.TritonListener.Backend.tcp_server.TritonServerTCP):
    def __init__(self, type, parentobj=None):
        ...
        self.running = True
        self.UDP_PORT_REC = Tilda.Driver.TritonListener.Backend.udp_server.TritonServerUDP.getfreeport()  # port used to receive messages
        self.UDP_PORT_CON = Tilda.Driver.TritonListener.Backend.udp_server.TritonServerUDP.getfreeport()  # port used to send messages
        self.UDP_PORT_ACK = Tilda.Driver.TritonListener.Backend.udp_server.TritonServerUDP.getfreeport()
    ...
    def udp_server_module(self):
        ...
        if Tilda.Driver.TritonListener.Backend.server_conf.SERVER_CONF.ENCRYPTION:
            fern = Fernet(TritonConfig.AESKey)
        while self.running:
            try:
    ...
    def subscribeTo(self, ip, port):
        ...
        if self.sendrequest(ip, port, msg_bytes=msg_data, msg_type=SERVER_CONF.SUB_REQUEST) is not None:
            return TritonRemoteObject(self, ip, port)
        else:
    ...
    def udp_send_data(self, ip, port, msg_bytes=None):
