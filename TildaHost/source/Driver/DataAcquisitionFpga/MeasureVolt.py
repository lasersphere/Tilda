...
class MeasureVolt(FPGAInterfaceHandling):
    ...
    '''read Indicators'''
    def getmeasVoltState(self, config):
        """
        gets the state of Voltage measurement Statemachine
        :return:int, state of Voltage measurement Statemachine
        """
        return self.ReadWrite(config.measVoltState).value



    '''write Indicators'''
    def setmeasVoltParameters(self, config, measVoltPars):
        """
        ...
        """
        self.ReadWrite(config.measVoltPulseLength25ns, measVoltPars['measVoltPulseLength25ns'])
        self.ReadWrite(config.measVoltTimeout10ns, measVoltPars['measVoltTimeout10ns'])
