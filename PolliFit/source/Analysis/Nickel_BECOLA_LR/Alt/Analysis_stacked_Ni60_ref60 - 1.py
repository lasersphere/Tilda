...
class NiAnalysis:
    ...
    def stack_files(self, files):
        ...
        for s in scalers:
            ...
            v_min = volcts[0][0]
            print(v_min)
            values = []     # voltage values to calc the mean from
            weights = []    # counts give weights
            sum = 0
            bg_sum = 0
            new_tuples = []
            ...
            for tup in volcts:
                if tup[0] < v_min + 1:
                    ...
                    v_min = tup[0]
                    new_tuples.append((np.average(values, weights=weights), sum, 1, bg_sum))
                    values = []  # voltage values to calc the mean from
                    weights = []  # counts give weights
                    sum = 0
                    bg_sum = 0
            ...
            for tup in new_tuples:
                vol.append(tup[0])
            steps = list(range(0, len(vol)))
            plt.plot(steps, vol, 'b.')
            plt.xlabel('step')
            plt.ylabel('Voltage in V')
            plt.title('DAC voltage bins')

            model = np.polyfit(steps, vol, 1)
            print(model)
            predict = np.poly1d(model)
            v = predict(steps)
            plt.plot(steps, v)
            plt.show()
            for i, tup in enumerate(new_tuples):
                new_tuples[i] = (predict(i), tup[1], tup[2], tup[3])
            ...
            for i, cts in enumerate(sumc):
                ...
                unc_norm.append(int((unc[i] / sumb[i] * np.mean(sumb))))
