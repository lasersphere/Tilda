...
class NiAnalysis():
    def __init__(self):
        ...
        self.run_name = 'Ni_StandartizedAnalysis'
        ...
        # data folder
        self.datafolder = os.path.join(self.workdir, 'SumsRebinned')  #Rebinned
        # results folder
        ...
        self.initial_par_guess = {'sigma': (34, True), 'gamma': (12, True),
        ...
        # standard settings for time gates per isotope.
        self.tof_mid = {'54Ni': 5.19, '55Ni': 5.23, '56Ni': 5.28, '58Ni': 5.36, '60Ni': 5.47, '62Ni': 5.56,
                        '64Ni': 5.65}  # mid-tof for each isotope (from fitting) 38, 47
        ...
        self.accVolt_set = 29850  #29850  # omit voltage sign, assumed to be negative TODO: Should be from files
        ...
        self.analysis_parameters = {'first_fit': 'from scratch' if self.do_the_fitting else load_results_from,
                                    'lineshape': self.run,
                                    'initial_par_guess': self.initial_par_guess,
                                    'gate_settings': 'individual' if self.use_individual_gates else 'global',
                                    'gate_global_presets':
                                    ...
                                    'cal_files_individual': self.cal_files_individual,
                                    'cal_scalers_individual': self.cal_scaler_individual,
                                    'do_not_cross_interpolation': self.do_not_cross,
                                    'wavg_uncertainties': self.combined_unc,
        ...
        self.matsuada_volts_d = 0.03  # V. ~standard dev after rebinning -> But that's more like a broadening effect
        self.wavemeter_wsu30_mhz_d = 1 * 2  # MHz. Kristians wavemeter paper. Factor 2 because of frequency doubling. For isoshift needs to be used twice (*sqrt(2)) bc ref/iso
        ...
        self.buncher_systematics = 2.0  # we see 1-3MHz larger IS for dc measurements vs bunched in stable isotopes. Until we understand this, add uncertainty.
        self.heliumneon_drift = 1 * 2  # MHz. According to datasheet of the SIOS China2 HeNe: 1h stability ~1MHz. Frequency doubled!
        self.hene_drift_longtime = 5  # MHz. According to datasheet of the SIOS China2 HeNe: Max drift ~5MHz. Used for calibration beamtime vs col/acol reference.
        ...
        self.summed_analysis_systematic = 2.4  # Uncertainty contribution of the summed data analysis
    ...
    def init_stuff(self):
        ...
        used_line = lineVar[0][0]
        cur.execute('''SELECT * FROM Lines WHERE lineVar = ? ''', (used_line,))  # get original line to copy from
    ...
    def fitting_initial_separate(self):
        if self.do_the_fitting:
            for sc in self.scaler_combinations:
                ...
                # get the scan voltage from the center fits
                self.all_centerFreq_to_scanVolt(self.isotopes_single, [scaler])
    ...
    def combine_single_scaler_results(self, parameter, isolist=None, calibrated=False, summed=False):
        ...
        for iso in isolist:
            ...
            for indx in range(len(file_names)):
                ...
                wavg, wavg_d, wstd, std, std_avg = self.calc_weighted_avg(val_arr, err_arr)  # TODO: use only fit errors for this?
                ...
                # for keeping track, find out which type of error was used
                pos = np.where(np.array([wavg_d, std, err_sc012])==d_fit)[0][0]
                err_names = ['wavg_d','std','err_sc012']

                # Append to combined results list.
                centers_combined.append(wavg)
                centers_combined_d_fit.append(d_fit)
                centers_which_err.append(err_names[pos])

                ''' uncertainties '''
                # statistic uncertainties
                stat_add = np.average(stat_without_fit)  # statistics except the fit. Should be same for all 3 PMTs
                if parameter == 'center_fits' and self.load_gate_analysis:
                    try:
                        gate_indx = self.gate_analysis_res[iso[:4]]['file_names'].index(file_names[indx])
                        gatewidth_stat = self.gate_analysis_res[iso[:4]]['scaler_012']['bunchwidth_std_0']['vals'][gate_indx]
                    except:
                        gatewidth_stat = 0
                else:
                    gatewidth_stat = 0
                centers_combined_d_stat.append(np.sqrt(d_fit**2 + stat_add**2 + gatewidth_stat**2))
                # systematic uncertainties
                d_syst = iso_r['{}{}'.format(sc_prefix, 1)][parameter]['d_syst'][indx]  # same for all scalers
                centers_combined_d_syst.append(d_syst)

            # calculate weighted avg of center fit and various error estimates over all files
        ...
        if parameter == 'center_fits':
            # get the scan voltage from the center fits
            self.all_centerFreq_to_scanVolt(isolist, ['scaler_c012'])
    ...
    def ion_energy_calibration(self):
        ...
        Calibration configuration is defined in init
        :return:
        """
        isolist = self.all_isotopes
        for sc in self.scaler_combinations+['scaler_c012']:
            logging.info('\n'
                         ...
                         .format(sc))
            # write the scaler to db for usage
            scaler_to_be_calib = self.update_scalers_in_db(sc)
            ...
            if 'absolute' in self.calibration_method:
                ref_sc = False
                if not self.cal_scaler_individual:
                    # all scalers are to be calibrated to the same value
                    ref_sc = self.update_scalers_in_db('scaler_012')  # self.scaler_name defines scaler used in calibration
                ...
                for iso in isolist:
                    ...
                    if self.cal_files_individual:
                        self.calibVoltageFunct(iso, scaler_to_be_calib,
                                               useOnly=self.calibration_method, userefscaler=ref_sc)
                    else:
                        self.calibVoltageFunct_global(iso, scaler_to_be_calib, useOnly=self.calibration_method)
            else:  # No Calibration
                ...
                for iso in isolist:
                    if self.cal_files_individual:
                        ...
                        self.calibVoltageFunct_global(iso, scaler_to_be_calib)
        ...
        for iso in isolist:
            for sc in self.results[iso].keys():
                if 'scaler' in sc:  # is a scaler key
                    ...
                    delta_rms_dict = {iso: {scaler: {'delta_ms_iso-{}'.format(refiso[:2]): {'vals': delta_rms,
                                                                                             'd_stat': zeros,  # TODO: Count as syst or stat?
                                                                                             'd_syst': delta_rms_d},
                                                     'avg_delta_ms_iso-{}'.format(refiso[:2]): {'vals': [avg_delta_rms],
                                                                                                 'd_stat': [0],
                                                                                                 'd_syst': [avg_delta_rms_d]}
                                                     }}}
                    TiTs.merge_extend_dicts(self.results, delta_rms_dict, overwrite=True, force_overwrite=True)
            #self.plot_parameter_for_isos_and_scaler(isolist, [sc], 'delta_ms_iso-{}'.format(refiso[:2]), digits=2)

        # self.plot_parameter_for_isos_vs_scaler(isolist, self.scaler_combinations + ['scaler_c012'],
        #                                        'avg_delta_ms_iso-{}'.format(refiso[:2]), digits=2)
        ...
        for iso in all_isos:
            ...
            final_vals_iso = {'center_fits':
                                      {'vals': iso_dict['avg_center_fits']['vals'],
                                       'd_fit_minmax': [min(iso_dict['center_fits']['d_fit']),
                                                        max(iso_dict['center_fits']['d_fit'])],
                              ...
                              'shift_iso-{}'.format(self.ref_iso[:2]):
                                      {'vals': iso_dict['avg_shift_iso-{}'.format(self.ref_iso[:2])]['vals'],
                                       'd_fit_minmax': [min(iso_dict['shift_iso-{}'.format(self.ref_iso[:2])]['d_fit']),
                                                        max(iso_dict['shift_iso-{}'.format(self.ref_iso[:2])]['d_fit'])],
                                       'd_stat': iso_dict['avg_shift_iso-{}'.format(self.ref_iso[:2])]['d_stat'],
                                       'd_syst': iso_dict['avg_shift_iso-{}'.format(self.ref_iso[:2])]['d_syst']},
                              'delta_ms_iso-{}'.format(self.ref_iso[:2]):
                                      {'vals': iso_dict['avg_delta_ms_iso-{}'.format(self.ref_iso[:2])]['vals'],
                                       'd_stat': iso_dict['avg_delta_ms_iso-{}'.format(self.ref_iso[:2])]['d_stat'],
                                       'd_syst': iso_dict['avg_delta_ms_iso-{}'.format(self.ref_iso[:2])]['d_syst']},
                              }
            if 'hfs_pars' in iso_dict.keys():
                # get final HFS values if applicable
                final_vals_iso['hfs_pars'] = iso_dict['hfs_pars']
    ...
    def chooseAndFitRuns(self, iso, reset=None):
        ...
        # set the standard timegates for this isotope
        t_gate_iso = iso[:4]
        ...
        while filearray.__len__() > 0:  # there are files that need fitting
            if not fileresults_dict:  # results still empty. No fitting has been done previously
                # make sure the line parameters are okay.
                par_guess_copy = self.initial_par_guess.copy()
                # This is the first time fitting, use the same pars for all files
                self.write_to_db_lines(self.run,
                ...
                if self.do_the_fitting:
                    ...
                    if self.use_individual_gates:
                        ...
                        # fit all files together.
                        BatchFit.batchFit(filearray, self.db, self.run, x_as_voltage=True, softw_gates_trs=None,
                                      guess_offset=True,
                                      save_to_folder=plot_folder)
            else:
                # this is a second round of fitting. Apparently some files need additional fixing of parameters
                for file in filearray:
                    # remove the previous fitresults:
                    con = sqlite3.connect(self.db)
                    cur = con.cursor()
                    cur.execute('''DELETE FROM FitRes WHERE file = ? AND iso LIKE ? AND run = ?''',
                                (file, db_like, self.run))
                    con.commit()
                    con.close()
                    # copy original line parameters
                    par_guess_copy = self.initial_par_guess.copy()
                    # Check the results from last fit
                    fitpars_last_try = fileresults_dict[file]['fitpars']
                    for par, vals in fitpars_last_try.items():
                        if par in par_guess_copy.keys():
                            par_guess_copy[par] = (par_guess_copy[par][0], vals[2])  # set par guess to previous fixed state for each par
                    # fix the first important thing
                    if fitpars_last_try.get('asy', (0, 0, 0))[1] is None:
                        par_guess_copy['asy'] = (par_guess_copy['asy'][0], True)
                        fileresults_dict[file]['fitpars']['asy'] = (par_guess_copy['asy'][0], 0, True)
                    elif fitpars_last_try.get('centerAsym', (0, 0, 0))[1] is None:
                        par_guess_copy['centerAsym'] = (par_guess_copy['centerAsym'][0], True)
                        fileresults_dict[file]['fitpars']['centerAsym'] = (par_guess_copy['centerAsym'][0], 0, True)
                    elif fitpars_last_try.get('IntAsym', (0, 0, 0))[1] is None:
                        par_guess_copy['IntAsym'] = (par_guess_copy['IntAsym'][0], True)
                        fileresults_dict[file]['fitpars']['IntAsym'] = (par_guess_copy['IntAsym'][0], 0, True)
                    elif fitpars_last_try.get('gamma', (0, 0, 0))[1] is None:
                        par_guess_copy['gamma'] = (par_guess_copy['gamma'][0], True)
                        fileresults_dict[file]['fitpars']['gamma'] = (par_guess_copy['gamma'][0], 0, True)
                    elif fitpars_last_try.get('sigma', (0, 0, 0))[1] is None:
                        par_guess_copy['sigma'] = (par_guess_copy['sigma'][0], True)
                        fileresults_dict[file]['fitpars']['sigma'] = (par_guess_copy['sigma'][0], 0, True)
                    # write the new parameters to the database
                    self.write_to_db_lines(self.run,
                                           sigma=par_guess_copy['sigma'],
                                           gamma=par_guess_copy['gamma'],
                                           asy=par_guess_copy['asy'],
            ...
            for indx, files in enumerate(filelist):
                if fileresults_dict.get(files, None) is None:
                    fileresults_dict[files] = {}
                ''' extract fitresults from db: '''
                con = sqlite3.connect(self.db)
                cur = con.cursor()
                ...
                try:
                    ...
                    for par, valstuple in parsdict.items():
                        if type(valstuple[2]) == list:  # A range was specified for this par
                            if valstuple[0]-valstuple[1] < valstuple[2][0] or valstuple[0]+valstuple[1] > valstuple[2][1]:
                                ...
                                filearray.append(files) if files not in filearray else filearray  # add file to list of 'fit again'
                                # Specify which pars did not fit well:
                                parsdict[par] = (valstuple[0], None,  valstuple[2])  # set error of the fitresult to False

                    ''' write the extracted fit parameters to a dict '''
                    fileresults_dict[files]['fitpars'] = parsdict
                    fileresults_dict[files]['file_date'] = file_date
                    fileresults_dict[files]['center_MHz'] = parsdict['center'][0]
                    fileresults_dict[files]['rChi'] = rChi[0][0]

                    ''' quantify uncertainties '''
                    # == fit uncertainites (changing per scaler):
                    # fit uncertainty
                    d_fit = parsdict['center'][1]
                    # statistic (fit-part) uncertainty part of the calibration
                    ...
                    d_hene_drift = self.heliumneon_drift
                    d_alignment = self.laserionoverlap_MHz_d
                    if self.load_gate_analysis:
                        # if the gate systematics were examined in detail load std from there
                        try:
                            gate_indx = self.gate_analysis_res[iso[:4]]['file_names'].index(files)
                            d_gatewidth_syst = self.gate_analysis_res[iso[:4]]['scaler_012']['bunchwidth_std_0']['vals'][
                                gate_indx]
                        except:
                            d_gatewidth_syst = self.softw_gate_d_syst
                    else:
                    ...
                    if 'cal' in iso:  # ion energy has been calibrated. Uncertainties from calibration
                        d_ion_energy_syst = self.diff_dopplers[iso[:4]] * \
                                       (self.results[iso][self.scaler_name]['acc_volts']['d_syst'][indx])  # Matsuada left out - should only broaden
                    else:  # not calibrated. Uncertainty from buncher potential
                        d_ion_energy_syst = self.diff_dopplers[iso[:4]]*(self.accVolt_set_d)  # not statistic, Matsuada left out again
                    d_wavemeter_nonlin = self.wavemeter_wsu30_mhz_d
                    d_lineshape = self.lineshape_d_syst

                    if self.cal_scaler_individual and self.cal_files_individual:
                        # if each scaler and file is calibrated separately add cal uncertainty to per-scaler-vals
                        d_fit = np.sqrt(d_fit**2 + d_ion_energy_stat**2)
                        d_stat = np.sqrt(d_fit**2 + d_hene_drift**2 + d_alignment**2 + d_gatewidth_syst**2)
                        d_syst = np.sqrt(d_ion_energy_syst**2 + d_wavemeter_nonlin**2 + d_lineshape**2)
                    elif not self.cal_scaler_individual and self.cal_files_individual:
                        # calibration for all scalers combined but separate per file: add to statistic
                        d_fit = np.sqrt(d_fit**2)
                        d_stat = np.sqrt(d_fit**2 + d_ion_energy_stat**2 + d_hene_drift**2 + d_alignment**2 + d_gatewidth_syst**2)
                        d_syst = np.sqrt(d_ion_energy_syst**2 + d_wavemeter_nonlin**2 + d_lineshape**2)
                    else:
                        # calibration for all scalers AND files combined --> all systematic
                        d_fit = np.sqrt(d_fit**2)
                        d_stat = np.sqrt(d_fit**2 + d_hene_drift**2 + d_alignment**2 + d_gatewidth_syst**2)
                        d_syst = np.sqrt(d_ion_energy_syst**2 + d_ion_energy_stat**2 + d_wavemeter_nonlin**2 + d_lineshape**2)

                    fileresults_dict[files]['center_MHz_dfit'] = d_fit
                    fileresults_dict[files]['center_MHz_dstat'] = d_stat
                    fileresults_dict[files]['center_MHz_dsyst'] = d_syst

                except Exception as e:  # Fit went wrong!
                    logging.error('################# fit error in file {} ################'.format(files))
                    if fileresults_dict[files].get('fitpars', None) is not None:
                        parsdict = {'gamma': (0, None, fileresults_dict[files]['fitpars'].get('gamma', (0,0,0))[2]),
                                    'sigma': (0, None, fileresults_dict[files]['fitpars'].get('sigma', (0,0,0))[2])}
                        if 'VoigtAsy' in self.run:
                            parsdict['asy'] = (0, None, fileresults_dict[files]['fitpars'].get('asy', (0,0,0))[2])
                        if 'AsymmetricVoigt' in self.run:
                            parsdict['centerAsym'] = (0, None, fileresults_dict[files]['fitpars'].get('centerAsym', (0,0,0))[2])
                            parsdict['IntAsym'] = (0, None, fileresults_dict[files]['fitpars'].get('IntAsym', (0,0,0))[2])
                    else:
                        parsdict = {'gamma': (0, None, self.initial_par_guess['gamma'][1]),
                                    'sigma': (0, None, self.initial_par_guess['sigma'][1])}
                        if 'VoigtAsy' in self.run:
                            parsdict['asy'] = (0, None, self.initial_par_guess['asy'][1])
                        if 'AsymmetricVoigt' in self.run:
                            parsdict['centerAsym'] = (0, None, self.initial_par_guess['centerAsym'][1])
                            parsdict['IntAsym'] = (0, None, self.initial_par_guess['IntAsym'][1])
                    # append to list of files that need fitting again!
                    filearray.append(files) if files not in filearray else filearray
                    fileresults_dict[files]['fitpars'] = parsdict
            filearray = np.array(filearray)

        # go back to standard fit parameters:
        self.write_to_db_lines(self.run,
                               sigma=self.initial_par_guess['sigma'],
                               gamma=self.initial_par_guess['gamma'],
                               asy=self.initial_par_guess['asy'],
        ...
        all_rundate = [fileresults_dict[f]['file_date'] for f in filelist]
        all_center_MHz = [fileresults_dict[f]['center_MHz'] for f in filelist]
        all_center_MHz_fiterrs = [fileresults_dict[f]['center_MHz_dfit'] for f in filelist]
        all_center_MHz_d = [fileresults_dict[f]['center_MHz_dstat'] for f in filelist]
        all_center_MHz_d_syst = [fileresults_dict[f]['center_MHz_dsyst'] for f in filelist]
        all_rChi = [fileresults_dict[f]['rChi'] for f in filelist]
    ...
    def getVoltDeviationFromAbsoluteTransFreq(self, iso, scaler_to_cal=None):
        ...
        rel_center_d_fit = np.array(center_freq_dict['d_fit'])
        ...
        ''' Uncertainties '''
        # fit uncertainties:
        d_centroid = rel_center_d_fit
        # statistic uncertainties:
        d_hene_shortterm = self.heliumneon_drift
        volt_dev_d_stat = np.sqrt(np.square(d_centroid)
                                  + np.square(d_hene_shortterm)
                                  )/self.diff_dopplers[iso[:4]]

        # systematics uncertainties:
        d_restframe_colacol = self.restframe_trans_freq[iso[:4]][1]
        d_hene_longterm = self.hene_drift_longtime
        d_wavemeter = self.wavemeter_wsu30_mhz_d
        d_lineshape = self.lineshape_d_syst
        d_alignment = self.laserionoverlap_MHz_d  # difference beamtime vs reference colacol alignment

        volt_dev_d_sys = np.sqrt(np.square(d_restframe_colacol)
                                 + np.square(d_hene_longterm)
                                 + np.square(d_wavemeter)
                                 + np.square(d_lineshape)
                                 + np.square(d_alignment)
                                 )/self.diff_dopplers[iso[:4]]
        volt_dev_d_sys = np.full(volt_dev_d_stat.shape, volt_dev_d_sys)  # make it an array

        # correction dict
        volt_dev_dict = {'vals': volt_dev.tolist(),
                         'd_stat': volt_dev_d_stat.tolist(),
                         'd_syst': volt_dev_d_sys.tolist()}
        self.results[iso][self.scaler_name]['voltage_deviation'] = volt_dev_dict

        return (volt_dev.mean(), volt_dev_d_stat.mean()+volt_dev_d_sys.mean())
    ...
    def calibVoltageFunct(self, isotope, scaler, useOnly=None, userefscaler=False):
        ...
        # For 58Ni reference
        ni58_numbers = np.array(self.results['58Ni']['file_numbers'])
        dates_58 = self.results['58Ni']['file_times']
        volt58_dev = self.results['58Ni'][ref_scaler]['voltage_deviation']['vals']
        volt58_dev_d = self.results['58Ni'][ref_scaler]['voltage_deviation']['d_stat']
        volt58_dev_d_syst = self.results['58Ni'][ref_scaler]['voltage_deviation']['d_syst']
        # For 60Ni reference
        ni60_numbers = np.array(self.results['60Ni']['file_numbers'])
        dates_60 = self.results['60Ni']['file_times']
        volt60_dev = self.results['60Ni'][ref_scaler]['voltage_deviation']['vals']
        volt60_dev_d = self.results['60Ni'][ref_scaler]['voltage_deviation']['d_stat']
        volt60_dev_d_syst = self.results['60Ni'][ref_scaler]['voltage_deviation']['d_syst']
        # For the isotope to be calibrated
        iso_times = self.results[isotope]['file_times']
        iso_names = self.results[isotope]['file_names']
        iso_numbers = np.array(self.results[isotope]['file_numbers'])
        ...
        # split the arrays in case there are point across which we don't want to interpolate
        iso_dates = np.split(iso_dates, [(iso_numbers > c).argmax() for c in self.do_not_cross])
        ref_58_dates = np.split(ref_58_dates, [(ni58_numbers > c).argmax() for c in self.do_not_cross])
        volt58_dev = np.split(volt58_dev, [(ni58_numbers > c).argmax() for c in self.do_not_cross])
        volt58_dev_d = np.split(volt58_dev_d, [(ni58_numbers > c).argmax() for c in self.do_not_cross])
        volt58_dev_d_syst = np.split(volt58_dev_d_syst, [(ni58_numbers > c).argmax() for c in self.do_not_cross])
        ref_60_dates = np.split(ref_60_dates, [(ni60_numbers > c).argmax() for c in self.do_not_cross])
        volt60_dev = np.split(volt60_dev, [(ni60_numbers > c).argmax() for c in self.do_not_cross])
        volt60_dev_d = np.split(volt60_dev_d, [(ni60_numbers > c).argmax() for c in self.do_not_cross])
        volt60_dev_d_syst = np.split(volt60_dev_d_syst, [(ni60_numbers > c).argmax() for c in self.do_not_cross])


        # use np.interp to assign voltage deviations to the requested run.
        interpolation_58 = np.empty(0)
        interpolation_58_d = np.empty(0)
        interpolation_58_d_syst = np.empty(0)
        interpolation_60 = np.empty(0)
        interpolation_60_d = np.empty(0)
        interpolation_60_d_syst = np.empty(0)
        for num, arr in enumerate(iso_dates):
            if arr.any():  # only if array is not empty
                interpolation_58 = np.concatenate((interpolation_58,
                                                   np.interp(iso_dates[num], ref_58_dates[num], [volt_offset-v for v in volt58_dev[num]])))
                interpolation_58_d = np.concatenate((interpolation_58_d,
                                                     np.interp(iso_dates[num], ref_58_dates[num], volt58_dev_d[num])))
                interpolation_58_d_syst = np.concatenate((interpolation_58_d_syst,
                                                          np.interp(iso_dates[num], ref_58_dates[num], volt58_dev_d_syst[num])))

                # same for 60 Nickel
                interpolation_60 = np.concatenate((interpolation_60,
                                                   np.interp(iso_dates[num], ref_60_dates[num],
                                                             [volt_offset - v for v in volt60_dev[num]])))
                interpolation_60_d = np.concatenate((interpolation_60_d,
                                                     np.interp(iso_dates[num], ref_60_dates[num], volt60_dev_d[num])))
                interpolation_60_d_syst = np.concatenate((interpolation_60_d_syst,
                                                          np.interp(iso_dates[num], ref_60_dates[num],
                                                                    volt60_dev_d_syst[num])))
        # join arrays back together
        iso_dates = np.concatenate(iso_dates)
        ref_58_dates = np.concatenate(ref_58_dates)
        volt58_dev = np.concatenate(volt58_dev)
        volt58_dev_d = np.concatenate(volt58_dev_d)
        volt58_dev_d_syst = np.concatenate(volt58_dev_d_syst)
        ref_60_dates = np.concatenate(ref_60_dates)
        volt60_dev = np.concatenate(volt60_dev)
        volt60_dev_d = np.concatenate(volt60_dev_d)
        volt60_dev_d_syst = np.concatenate(volt60_dev_d_syst)
        ...
        voltdict = {isotope_cal: {scaler: {'acc_volts': {'vals': voltcorrect.tolist(),
                                  ...
                                  'file_numbers': iso_numbers.tolist(),
    ...
    def calibVoltageFunct_global(self, isotope, scaler, useOnly=None):
        """
        return a voltage calibration based on a datetime object using the offset from literature IS and the deviations
        from constant transition frequency.
        :param isotope:
        ...
        """
        scaler = self.update_scalers_in_db(scaler)
        if self.cal_scaler_individual:
            cal_from_scaler = scaler
        else:
            cal_from_scaler = 'scaler_012'
        ...
        ni58_numbers = np.array(self.results['58Ni']['file_numbers'])
        dates_58 = self.results['58Ni']['file_times']
        volt58_dev = self.results['58Ni'][cal_from_scaler]['voltage_deviation']['vals']
        volt58_dev_d = self.results['58Ni'][cal_from_scaler]['voltage_deviation']['d_stat']
        volt58_dev_d_syst = self.results['58Ni'][cal_from_scaler]['voltage_deviation']['d_syst']
        # For 60Ni reference
        ni60_numbers = np.array(self.results['60Ni']['file_numbers'])
        dates_60 = self.results['60Ni']['file_times']
        volt60_dev = self.results['60Ni'][cal_from_scaler]['voltage_deviation']['vals']
        volt60_dev_d = self.results['60Ni'][cal_from_scaler]['voltage_deviation']['d_stat']
        volt60_dev_d_syst = self.results['60Ni'][cal_from_scaler]['voltage_deviation']['d_syst']
        # For the isotope to be calibrated
        iso_times = self.results[isotope]['file_times']
        iso_names = self.results[isotope]['file_names']
        iso_numbers = np.array(self.results[isotope]['file_numbers'])
        ...
        # fit a constant to the calibrated voltages
        def const_off(x, voff):
            return voff
        if '58' in useOnly:
            # use only the 58 Nickel interpolation
            x = ref_58_dates
            y = volt_offset - np.array(volt58_dev)
            y_err = volt58_dev_d
            d_syst = volt58_dev_d_syst
        elif '60' in useOnly:
            # use only the 60 Nickel interpolation
            x = ref_60_dates
            y = volt_offset - np.array(volt60_dev)
            y_err = volt60_dev_d
            d_syst = volt60_dev_d_syst
        else:  # standard case
            # combine 58 and 60 Nickel references
            x = np.append(ref_58_dates, ref_60_dates)
            y = volt_offset - np.append(volt58_dev, volt60_dev)
            y_err = np.append(volt58_dev_d, volt60_dev_d)

            d_syst = max(volt58_dev_d_syst, volt60_dev_d_syst)

        popt, pcov = curve_fit(const_off, x, y, y[0], sigma=y_err, absolute_sigma=True)
        perr = np.sqrt(np.diag(pcov))

        voltcorrect = np.full(iso_numbers.shape, popt[0])
        voltcorrect_d = np.full(iso_numbers.shape, perr[0])
        voltcorrect_d_syst = np.full(iso_numbers.shape, d_syst[0])
    ...
    def extract_iso_shift_interp(self, isotope, reference, scaler, calibrated=True):
        ...
        iso_center_d_stat = self.results[isotope][scaler]['center_fits']['d_stat']  # statistic unc. for IS added new below
        ...
        ref_numbers = np.array(self.results[ref]['file_numbers'])
        ...
        # split the arrays in case there are point across which we don't want to interpolate
        iso_dates = np.split(iso_dates, [(iso_numbers > c).argmax() for c in self.do_not_cross])
        ref_dates = np.split(ref_dates, [(ref_numbers > c).argmax() for c in self.do_not_cross])
        ref_center = np.split(ref_center, [(ref_numbers > c).argmax() for c in self.do_not_cross])
        ref_center_d_fit = np.split(ref_center_d_fit, [(ref_numbers > c).argmax() for c in self.do_not_cross])
        ref_center_d_stat = np.split(ref_center_d_stat, [(ref_numbers > c).argmax() for c in self.do_not_cross])
        ref_center_d_syst = np.split(ref_center_d_syst, [(ref_numbers > c).argmax() for c in self.do_not_cross])

        # use np.interp to assign voltage deviations to the requested run.
        ref_center_interp = np.empty(0)
        ref_center_d_fit_interp = np.empty(0)
        ref_center_d_stat_interp = np.empty(0)
        ref_center_d_syst_interp = np.empty(0)

        for num, arr in enumerate(iso_dates):
            if arr.any():  # only if array is not empty
                ref_center_interp = np.concatenate((ref_center_interp,
                                                    np.interp(iso_dates[num], ref_dates[num], ref_center[num])))
                ref_center_d_fit_interp = np.concatenate((ref_center_d_fit_interp,
                                                          np.interp(iso_dates[num], ref_dates[num], ref_center_d_fit[num])))
                ref_center_d_stat_interp = np.concatenate((ref_center_d_stat_interp, np.interp(iso_dates[num], ref_dates[num], ref_center_d_stat[num])))
                ref_center_d_syst_interp = np.concatenate((ref_center_d_syst_interp, np.interp(iso_dates[num], ref_dates[num], ref_center_d_syst[num])))


        # join arrays back together
        iso_dates = np.concatenate(iso_dates)
        ref_dates = np.concatenate(ref_dates)
        ref_center = np.concatenate(ref_center)
        ref_center_d_fit = np.concatenate(ref_center_d_fit)
        ref_center_d_stat = np.concatenate(ref_center_d_stat)
        ref_center_d_syst = np.concatenate(ref_center_d_syst)
        ...
        hene_drift_d_stat = self.heliumneon_drift
        alignment_d_stat = self.laserionoverlap_MHz_d  # Can't always guarantee the alignment is still the same between files...
        if self.load_gate_analysis:
            try:
                gatewidth_d_stat = []
            ...
            gatewidth_d_stat = self.softw_gate_d_syst
        # combined statistic:
        iso_shifts_d_stat = np.sqrt(iso_shifts_d_fit**2
                                    + ion_energy_d_stat**2
                                    + hene_drift_d_stat**2
                                    + alignment_d_stat**2
                                    + gatewidth_d_stat**2)
        ...
        # systematic errors:
        ion_energy_d_syst = np.array(iso_volts_d_syst) * delta_diff_doppler  # calibration for iso is interpolated from ref, so only that value needs to go in here
        laser_freq_d_syst = self.wavemeter_wsu30_mhz_d * np.sqrt(2)  # Laser is changed between isotopes.  TODO: HeNe drift?
        # lineshape systematics cancel for isotope shift (tested and confirmed)
        buncher_d_syst = self.buncher_systematics
        if 'sum' in isotope:
            summed_d_syst = self.summed_analysis_systematic
        else:
            summed_d_syst = 0
        ...
        iso_shift_avg, wavg_d, wstd, std, std_avg = self.calc_weighted_avg(iso_shifts, iso_shifts_d_stat)
        if self.combined_unc == 'wavg_d':
    ...
    def create_stacked_files(self, calibration_per_file=False):
        ...
        for iso in self.all_isotopes:
            bin = self.binsize.get(iso, 1)
            scan_range = self.scanrange.get(iso, (-2000, 2000))
            file_range = self.filerange.get(iso, None)
            files_to_stack, file_nos, file_times = self.pick_files_from_db_by_type_and_num('%{}%'.format(iso), selecttuple=file_range)
            self.stack_runs('{}{}'.format(iso, c), files_to_stack, scan_range, binsize=bin, bake_in_calib=calibration_per_file)
    ...
    def stack_runs(self, isotope, files, volttuple, binsize, bake_in_calib=False):
        ...
        # self.summed_time_gates[isotope] = self.stack_time_projections(isotope, files)
        self.addfiles_trs(isotope, files, volttuple, binsize, bake_in_calib)

    def get_timegates_from_sumfiles(self):
        timegates_per_iso = {}
        for iso in self.all_isotopes:
            # get a list of all files for this isotope
            files_to_stack, file_nos, file_times = self.pick_files_from_db_by_type_and_num('%{}%'.format(iso))
            # sum the timeprojections and fit. 'scaler_#': {'sigma':(v,d), 'center':(v,d), 'ampl':(v,d), 'offset':(v,d)}
            fitres_per_scaler = self.stack_time_projections(iso, files_to_stack)

            tof_mid = fitres_per_scaler['scaler_0']['center'][0]/100  # mid-tof for isotope (from fitting)
            tof_delay = [0,
                         fitres_per_scaler['scaler_1']['center'][0]/100-tof_mid,
                         fitres_per_scaler['scaler_2']['center'][0]/100-tof_mid]
            tof_sigma_per_sc = [fitres_per_scaler['scaler_{}'.format(sc)]['sigma'][0] for sc in range(3)]
            tof_sigma_d_per_sc = [fitres_per_scaler['scaler_{}'.format(sc)]['sigma'][1] for sc in range(3)]
            tof_sigma = self.calc_weighted_avg(tof_sigma_per_sc, tof_sigma_d_per_sc)[0]/100

            if not self.is_dc_data:
                timegates_per_iso[iso] = {'tof_mid': tof_mid, 'tof_delay': tof_delay, 'tof_sigma': tof_sigma}
            else:
                timegates_per_iso[iso] = {'tof_mid': 5.12, 'tof_delay': [0,0,0], 'tof_sigma': 10.24}

        return timegates_per_iso

    def stack_time_projections(self, isotope, filelist):
        zeroarr_sc = np.zeros(1024)  # array for one scaler
        tproj = np.array([zeroarr_sc.copy(), zeroarr_sc.copy(), zeroarr_sc.copy()])
        ...
        for sc_no in range(3):
            ...
            timeproj_res['scaler_{}'.format(sc_no)] = {'ampl': (ampl, ampl_d),
                                                       'sigma': (sigma, sigma_d),
                                                       'center': (center, center_d),
                                                       'offset': (offset, offset_d)}
    ...
    def addfiles_trs(self, iso, filelist, voltrange, binsize, bake_in_calib=False):
        ...
        # set the standard timegates for this isotope (not necessary for summing, but for file overview)
        t_gate_iso = iso[:4]
        self.update_gates_in_db(t_gate_iso,
                                self.tof_mid[t_gate_iso],
                                self.tof_width_sigma * self.tof_sigma[t_gate_iso],
        ...
        for files in filelist:
            ...
            filepath = os.path.join(self.datafolder, files)
            spec = XMLImporter(path=filepath, softw_gates=(self.db, self.run))
            ...
            for track, trackdata in enumerate(trs_data):
                ...
                volt_proj_cts = spec.cts[track].sum(axis=0)
                plt.plot(voltage_x, volt_proj_cts, drawstyle='steps', label=filenumber)
                ...
                for scaler, scalerdata in enumerate(trackdata):
                    ...
                    for step, stepdata in enumerate(scalerdata):
                        ...
                        if voltrange[0] <= volt_c <= voltrange[1]:  # only use if inside desired range
                            ...
                            nOfScans_arr[voltind] += nOfScans
        # plot all original voltage projections (to get an overview of the data before summing):
        plt.xlabel('Scan Voltage/V')
        plt.ylabel('Counts')
        plt.legend(title='Measurements', bbox_to_anchor=(1.04, 0.5), loc="center left")
        plt.title('Individual Nickel {} Measurements.'.format(str(iso[:2])))
        if self.save_plots_to_file:
            filename = 'all_volt_proj_{}_{}tracks'.format(str(iso), str(nOfTracks))
            folder = 'summed'
        ...
        # Just for reference: plot the summed data as well as the background and nOfSCans
        mid_tof = int(self.tof_mid[iso]*100)
        range_tof = int(self.abs_gatewidth[iso]*100)
        for tr in range(nOfTracks):
            # plt.plot(tr_volt[tr],
            #          tr_cts_trs[tr][:,:,mid_tof-range_tof:mid_tof+range_tof].sum(axis=(0, 2))/tr_nOfScans[tr],
            #          drawstyle='steps', c='k', label='cts')
            plt.plot(tr_volt[tr],
                     tr_cts_trs[tr][:, :, mid_tof - range_tof:mid_tof + range_tof].sum(axis=(0, 2)) / tr_bg_trs[tr][:,:,mid_tof-range_tof:mid_tof+range_tof].sum(axis=(0, 2)),
                     drawstyle='steps', c='r', label='cts')
            # plt.plot(tr_volt[tr], tr_bg_trs[tr][:,:,mid_tof-range_tof:mid_tof+range_tof].sum(axis=(0, 2)), drawstyle='steps', c='g', label='bg')
            # plt.plot(tr_volt[tr], tr_nOfScans[tr], drawstyle='steps', c='r', label='scans')
        # Make the plot nice:
        plt.xlabel('Scan Voltage/V')
        plt.ylabel('Counts')
        plt.xlim((tr_volt[0][0], tr_volt[-1][-1]))
        plt.legend(bbox_to_anchor=(1.04, 0.5), loc="center left")
        plt.title('Summed Nickel {} Measurements. No Normalization.'.format(str(iso[:2])))
        if self.save_plots_to_file:
            filename = 'add_raw_{}_{}tracks'.format(str(iso), str(nOfTracks))
            folder = 'summed'
            path_add = '{}\\'.format(folder)
            plot_folder = self.resultsdir + path_add
            if not os.path.exists(plot_folder):
    ...
    def addfiles_trs_old(self, iso, filelist, voltrange, binsize, bake_in_calib=False):
        ...
        cts_trs_array = cts_trs_array[:, :, include_indx, :][:, :, 1:-1, :]
        bg_trs_array = bg_trs_array[:, :, include_indx, :][:, :, 1:-1, :]

        normalize = True
        if normalize:
            # before normalization, calculate the errors
            err_trs_array = np.sqrt(np.square(np.sqrt(cts_trs_array) / bg_trs_array) +
                                    np.square(cts_trs_array / np.square(bg_trs_array) * np.sqrt(bg_trs_array))) \
                            * bg_trs_array.mean()
            err_trs_array = np.sqrt(cts_trs_array)
            A = bg_trs_array.mean()
            A0 = bg_trs_array[:, 0, :, :].mean()
            A1 = bg_trs_array[:, 1, :, :].mean()
            A2 = bg_trs_array[:, 2, :, :].mean()
            sc_shape = bg_trs_array.shape[2:]
            # err_trs_array = np.sqrt(cts_trs_array) / bg_trs_array *np.array([np.full(sc_shape, A0),
            #                                                         np.full(sc_shape, A1),
            #                                                         np.full(sc_shape, A2)])
            err_trs_array = np.sqrt(np.square(np.sqrt(cts_trs_array) / bg_trs_array) +
                                    np.square(cts_trs_array / np.square(bg_trs_array) * np.sqrt(bg_trs_array))) \
                            * np.array([np.full(sc_shape, A0),
                                        ...
                                        np.full(sc_shape, A2)]) # bg_trs_array.mean()  # scale factor bg_mean to reduce int rounding errors
            # now normalize
            # cts_trs_array = cts_trs_array / bg_trs_array * bg_trs_array.mean()
            co0 = cts_trs_array[0, 0, 30, 547]
            co1 = cts_trs_array[0, 1, 30, 547]
            co2 = cts_trs_array[0, 2, 30, 547]
            cts_trs_array = cts_trs_array / bg_trs_array *np.array([np.full(sc_shape, A0),
                                                                    np.full(sc_shape, A1),
                                                                    np.full(sc_shape, A2)]) # bg_trs_array.mean() # scale factor bg_mean to reduce int rounding errors
        else:
            err_trs_array = np.sqrt(cts_trs_array)
        ...
        c1 = cts_trs_array[0, 1, 30, 547]
        c2 = cts_trs_array[0, 2, 30, 547]
        ear0 = err_trs_array[0, 0, 30, 547]
        ear1 = err_trs_array[0, 1, 30, 547]
        ear2 = err_trs_array[0, 2, 30, 547]
        bg0 = bg_trs_array[0, 0, 30, 547]
        bg1 = bg_trs_array[0, 1, 30, 547]
        bg2 = bg_trs_array[0, 2, 31, 547]
        ...
        # plt.plot(np.arange(len(real_volt_arr)), volt_arr, 'k-')
        # plt.plot(np.arange(len(real_volt_arr)), _line(np.arange(len(real_volt_arr)), real_stepsize_v, real_start_v), 'r--')
        # plt.show()
        ...
        for track in range(nOfTracks):
            ...
            # same for the errors:
            err_tuples_list = []
            for index, cts in np.ndenumerate(err_trs_array[track]):
                # Loop over error array and extract index + cts and combine them to a tuple.
                err_point_tuple = index + (int(round(cts)),)
                err_tuples_list.append(err_point_tuple)  # append tuple to list
            track_err_list.append(
        ...
        err_tuples_arr = np.stack(track_err_list)
        # Create voltage projection
        track_data_list = []
        for track in range(nOfTracks):
            voltage_projections = []
    ...
    def extract_radius_from_factors(self, iso, ref, scaler=None, isoshift=None, printErrCont=False):
        """
        ...
        Z = self.nOfElectrons
        m_iso = m_iso - Z * Physics.me_u + self.ionization_energy * sc.e / (sc.atomic_mass * sc.c ** 2)
        m_ref = m_ref - Z * Physics.me_u + self.ionization_energy * sc.e / (sc.atomic_mass * sc.c ** 2)
        ...
        if scaler is not None:
            ...
            if iso[:2] == ref[:2]:
                ...
                for indx, file in enumerate(files):
                    ...
                    F_cont = F_d * (iso_shift[
                                        indx] - mu * M_alpha) / F ** 2  # contribution of the field-shift factor uncertainty
                    IS_cont = np.sqrt(
                        iso_shift_d[indx] ** 2 + iso_shift_d_syst[indx] ** 2) / F  # contribution of the isoshift value
                    # combine all:
                ...
                mu_cont = mu_d * (alpha - M_alpha / F)
                M_cont = M_alpha_d * mu / F  # contribution of the mass-shift factor uncertainty
                F_cont = F_d * (avg_iso_shift - mu * M_alpha) / F ** 2  # contribution of the field-shift factor uncertainty
                IS_cont = np.sqrt(avg_iso_shift_d ** 2 + avg_iso_shift_d_syst ** 2) / F  # contribution of the isoshift value
                # combine all:
    ...
    def get_gate_analysis_results(self, iso, file):
        ...
        if self.gate_analysis_res[iso].get('recommended_delay_SNR', None) is not None:
            # Wehen all scalers were analyzed I extracted recommended delays also
            delay = self.gate_analysis_res[iso]['recommended_delay_SNR']
        else:
            # Very well known and not necessary to do individual
            delay = None
        # calculate the gatewidth
        gate = sigma*self.tof_width_sigma
        # set the new gates in db
    ...
    def plot_parameter_for_isos_and_scaler(self, isotopes, scaler_list, parameter,
        ...
        for sc in scaler_list:
            ...
            for i in range(len(isotopes)):
                ...
                std = '{:.0f}'.format(10**digits*std)
                if off != 0:
                    plt_label = '{0} {1:.{7:d}f}({2})[{3}]{4} (offset: {5}{6})'\
                        .format(labelstr, wavg, wavg_d, std,  unit, off, unit, digits)
                else:
                    plt_label = '{0} {1:.{5:d}f}({2})[{3}]{4}'\
                        .format(labelstr, wavg, wavg_d, std, unit, digits)
                if plotstyle == 'band':
    ...
    def make_final_plots(self):
        ...
        self.plot_centroids(isolist_cal,
                            ['scaler_0', 'scaler_1', 'scaler_2', 'scaler_012', 'scaler_c012'],
    ...
    def plot_results_table(self, ref_iso):
        ...
        header_labels = ('avg_center_fits', 'avg_shift_iso-{}'.format(ref_iso[:2]), 'avg_delta_ms_iso-{}'.format(ref_iso[:2]))
        prop_digits = (1, 1, 3)
