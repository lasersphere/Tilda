...
# get all used configs
configs = {}

# get all needed files, mentioned in all configs, sorted by run:
configs[final_2017_run] = {}
for iso in isotopes:
    con = sqlite3.connect(db)
    cur = con.cursor()
    cur.execute(
        '''SELECT config, val, statErr, rChi FROM Combined WHERE iso = ? AND run = ? AND parname = ? ''',
        (iso, final_2017_run, 'shift'))
    data = cur.fetchall()
    con.close()
    if len(data):
        config, val, statErr, rChi = data[0]
        # print('%s \t %s \t %s \t %s \t %s \n %s' % (run, iso, val, statErr, rChi, config))
        configs[final_2017_run][iso] = ast.literal_eval(config)
    else:
        print('error: run: %s iso: %s no config found' % (final_2017_run, iso))
print('58Ni config:', configs[final_2017_run]['58_Ni'])
# sort files in cfg by ref or iso
all_iso_shift_files = {}
all_iso_shift_files_by_iso = {}
for run_name in [final_2017_run]:
    all_iso_shift_files[run_name] = {'refs': [], 'isoFiles': []}
    all_iso_shift_files_by_iso[run_name] = {'refs': {}, 'isoFiles': {}}
    for iso, cfg in configs[final_2017_run].items():
...
if plot_comparisons:
    ...
    MPLPlotter.plot_par_from_combined(db, runs_to_compare,
                                      ...
                                      use_syst_err_only=False,
                                      use_full_error=False,
                                      save_path=os.path.join(store_path_comparison_plots, '2016_2017_all_runs_no_Steudel.pdf'),
                                      force_xlim=(57, 71),
                                      fontsize_ticks=font_size_plts, fig_size=fig_size_plots
                                      )
...
q_moments = []
magn_moments = []
if print_moments_latex:
    print('\hline \hline')
    print('A & I & $A_u$ / MHz & $A_l$  / MHz & $A_u$/$A_l$	& $B_u$  / MHz & $B_l$ / MHz & $B_u$/$B_l$\\\\')
    print('\hline')
else:
...
for run in a_fac_runs:
    for iso, a_low in sorted(al[run].items()):
        ...
        if a_low[0]:
            ...
            if print_moments_latex:
                print(
                    '%s & %s & %.2f(%.0f)[%.0f] & %.2f(%.0f)[%.0f] & %.3f(%.0f) &'
                    '%.2f(%.0f)[%.0f] & %.2f(%.0f)[%.0f] & %.3f(%.0f) \\\\' % (
                        iso[:2], str(Fraction(nucl_spin)),
                        a_up[0], a_up[1] * 100, a_up[2] * 100,
                        a_low[0], a_low[1] * 100, a_low[2] * 100,
                        ratio, delta_ratio,
                        b_up[0], b_up[1] * 100, b_up[2] * 100,
                        b_low[0], b_low[1] * 100, b_low[2] * 100,
                        b_ratio, delta_b_ratio
                    )
                )
